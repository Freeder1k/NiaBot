...
async def _create_player_embed(p: MinecraftPlayer) -> Embed | None:
    stats: PlayerStats = await common.api.wynncraft.v3.player.stats(common.utils.misc.format_uuid(p.uuid), full_result=True)
    ...
    dungeons_tb = common.utils.tableBuilder.TableBuilder.from_str("l   r")
        ...
        async with event.channel.typing():
            ...
            if len(user_str) <= 16 and _USERNAME_RE.fullmatch(user_str):
                ...
                if len(user_str) == 32 and _UUID_RE.fullmatch(user_str):
                    ...
                    await common.utils.discord.send_error(event.channel, f"Couldn't parse player ``{event.args[1]}``.")
                    return

            if p is None:
                await common.utils.discord.send_error(event.channel, f"Couldn't find player ``{event.args[1]}``.")
                return
            ...
            try:
                ...
                await common.utils.discord.send_error(event.channel, f"{escape_markdown(p.name)} is not a wynncraft player!")
