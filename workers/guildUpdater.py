...
try:
    if os.path.exists(f"data/{botConfig.GUILD_NAME}.json"):
        with open(f"data/{botConfig.GUILD_NAME}.json", 'r') as _f:
...
async def _store_guild():
    with open(f"data/{botConfig.GUILD_NAME}.json", "w") as f:
...
async def _log_member_updates(guild_now: common.types.wynncraft.GuildStats):
    ...
    for uuid in joined_uuids:
        await common.logging.log_member_join(joined.get(uuid, '*unknown*'), uuid)
    for uuid in left_uuids:
        await common.logging.log_member_leave(left.get(uuid, '*unknown*'), uuid)
...
async def update_guild():
    try:
        ...
        try:
            ...
            common.logging.error(f"Guild {botConfig.GUILD_NAME} not found. Disabling guild update loop.")
            update_guild.stop()
            return
